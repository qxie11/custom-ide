...
import type {EditorSettings, ThemeType} from '../model/types';
    ...
    <div className="p-4 space-y-6 h-full overflow-y-auto text-sm">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-accent/5">
          <Label htmlFor="font-size" className="text-foreground/80">
            Font Size
          </Label>
          <Input
            id="font-size"
        ...
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-accent/5">
          <Label htmlFor="auto-save" className="text-foreground/80">
            Auto Save
          </Label>
          <Switch id="auto-save" checked={localSettings.autoSave} onCheckedChange={checked => setLocalSettings(s => ({...s, autoSave: checked}))} />
        </div>
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-accent/5">
          <Label htmlFor="word-wrap" className="text-foreground/80">
            Word Wrap
          </Label>
          <Switch id="word-wrap" checked={localSettings.wordWrap === 'on'} onCheckedChange={checked => setLocalSettings(s => ({...s, wordWrap: checked ? 'on' : 'off'}))} />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-accent/5">
          <Label htmlFor="color-theme" className="text-foreground/80">
            Color Theme
          </Label>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between p-2 rounded-md hover:bg-accent/5">
          <Label htmlFor="format-on-save" className="text-foreground/80">
            Format On Save
          </Label>
          <Switch id="format-on-save" checked={localSettings.formatOnSave} onCheckedChange={checked => setLocalSettings(s => ({...s, formatOnSave: checked}))} />
        </div>
      </div>

      <div className="pt-4">
        <Button size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleApplyClick}>
          Apply Settings
        </Button>
      </div>
    </div>
