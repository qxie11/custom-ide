  ...
  return (
    <div className="p-2 space-y-3 h-full overflow-y-auto text-sm">
      <div className="relative">
        <Input placeholder="Search" value={query} onChange={e => setQuery(e.target.value)} className="pr-20 bg-input" />
      ...
      <div>
        <Input placeholder="files to include" value={filesToInclude} onChange={e => setFilesToInclude(e.target.value)} className="bg-input text-xs h-8" />
        <Input placeholder="files to exclude" value={filesToExclude} onChange={e => setFilesToExclude(e.target.value)} className="bg-input text-xs h-8 mt-1" />
      </div>

      <div className="text-xs text-muted-foreground">
        {searchResults.length > 0 ? `${searchResults.reduce((sum, r) => sum + r.matches.length, 0)} results in ${searchResults.length} files` : 'No results found.'}
      </div>
          ...
          <AccordionItem key={file.id} value={file.id} className="border-b-0">
            ...
            <AccordionContent className="pl-4 text-xs">
              {matches.map((match, index) => (
                <div key={`${file.id}-${match.line}-${index}`} className="flex py-1 hover:bg-accent/10 rounded-sm cursor-pointer" onClick={() => handleResultClick(file)}>
